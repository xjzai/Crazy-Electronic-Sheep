...
- 当前代码已落地 `issue #2《MVP 工程底座与新档开局》` 与 `issue #3《第一图自动产出与核心 HUD》`
...
- 已实现：`Boot Coordinator`、本地存档读取入口、新档初始化、`map_01` 主场景骨架、双地图最小状态骨架、第一图自动产出、核心 HUD（当前资源 / 全局总秒产 / 羊钻占位数值）
- 尚未实现：购买、科技、第二图正式玩法、离线收益、完整地图切换与跨地图解锁流程
